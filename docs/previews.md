...
Both writing workflows require the repository variable `PREVIEWFORGE_REMOTE_ENABLED=true`. Their repository-scoped Actions token is separate from the laptop's `gh` session. Every laptop GitHub call verifies the active account is HickoDev. Publication refuses a public repository or public/unrelated package and checks private package visibility after pushing. The workflow token needs no laptop or Kubernetes credentials.
